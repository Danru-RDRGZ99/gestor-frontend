...
def PrestamosView(page: ft.Page, api: ApiClient):
    ...
    # Los mismos controles para ambos layouts, se adaptan con responsive design
    ...
    # ---------------------------------
    # Lógica de renderizado y filtros (MISMO CÓDIGO PARA AMBOS)
    # ---------------------------------
    def render_recursos():
        ...
        if not recursos:
            ...
            for r in recursos:
                if isinstance(r, dict):
                    # USAR SIEMPRE EL MISMO TILE, SE ADAPTA INTERNAMENTE
    ...
    # ========================================================================
    # TILES ÚNICOS QUE SE ADAPTAN A MÓVIL/WEB
    # ========================================================================

    def recurso_tile(r: dict):
        """Tile único para recursos que se adapta automáticamente"""
        ...
        if state["is_mobile"]:
            # DISEÑO MÓVIL - Más compacto pero mantiene esencia
            title = ft.Text(
            ...
            return Card(
                ft.Container(
                    ft.Column([
                        ...
                        ft.Container(btn, alignment=ft.alignment.center_right)
                    ], spacing=8),
                    padding=12
                )
            )
            ...
            subtitle = ft.Text(f"Plantel: {plantel.get('nombre', '-')} · Lab: {lab.get('nombre', '-')}", 
                             size=12, opacity=0.8)
    ...
    def solicitud_tile(s: dict):
        """Tile único para solicitudes que se adapta automáticamente"""
        ...
        if state["is_mobile"]:
            ...
            if is_admin:
                ...
                admin_info = ft.Text(f"Solicitante: {solicitante_nombre}", size=12, color=ft.Colors.GREY_600)
            ...
            # Acciones para admin (en móvil se muestran como botones)
            admin_actions = None
            ...
            if is_admin:
                if current_status == 'pendiente':
                    ...
                    admin_actions = Primary("Marcar Entregado", expand=True,
                                          on_click=lambda _, _id=s['id']: update_loan_status(_id, 'entregado'))
                elif current_status == 'entregado':
                    admin_actions = Primary("Marcar Devuelto", expand=True,
                                          on_click=lambda _, _id=s['id']: update_loan_status(_id, 'devuelto'))

            if admin_actions:
                content.append(ft.Container(admin_actions, margin=ft.margin.only(top=8)))
            ...
            # DISEÑO WEB - Original
            title = ft.Text(f"Solicitud #{solicitud_id} · {recurso_tipo} #{recurso_id_val}", 
                           size=15, weight=ft.FontWeight.W_600)
            timeline = ft.Text(f"Pedido: {format_iso_date(s.get('created_at'))} · Devolución: {format_iso_date(s.get('fin'))}", 
                             size=11)
    ...
    def admin_recurso_tile(r: dict):
        """Tile único para gestión de recursos que se adapta automáticamente"""
        lab_id = r.get('laboratorio_id')
        lab = next((l for l in labs_cache if l.get('id') == lab_id), {})
        plantel_id = lab.get('plantel_id')
        plantel = next((p for p in planteles_cache if p.get('id') == plantel_id), {})
        ...
        if state["is_mobile"]:
            ...
            actions = ft.Row([
                Tonal("Editar", on_click=lambda e, _r=r: edit_recurso_click(_r)),
                Danger("Eliminar", on_click=lambda e, _r=r: delete_recurso_click(_r)),
            ], spacing=6)
            ...
            return Card(
                ft.Container(
                    ft.Column([
                        ...
                        ft.Container(actions, alignment=ft.alignment.center_right)
                    ], spacing=8),
                    padding=12
                )
            )
    ...
    # ========================================================================
    # FUNCIONES DE GESTIÓN (MISMAS PARA AMBOS)
    # ========================================================================
    ...
    def mobile_layout():
        """Layout móvil que mantiene la esencia del web pero adaptado"""
